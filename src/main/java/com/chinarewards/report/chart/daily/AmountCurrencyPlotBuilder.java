...
/**
 * Plot for daily report's spending amount.
 * 
 * @author Cyril
 * @since 1.3.0 2010-01-17
 */
public class AmountCurrencyPlotBuilder extends AbstractPlotBuilder implements PlotBuilder {

	private TimeSeries series = new TimeSeries("Amount");
	...
	public XYPlot buildPlot() {
		...
		NumberAxis axis = new NumberAxis("RMB");
		...
		for (int i = 0; i < mavDays.length; i++) {
			...
			TimeSeries mav = MovingAverage.createMovingAverage(series,
					"Amount (" + mavDay + "-Day MA)", mavDay, mavDay);
	...
	public double getDoubleValue(ResultSet rs) throws SQLException {
		return rs.getDouble("amountcurrency_sum");
	}
