...
/**
 * Plot for daily report's transaction count.
 * 
 * @author Cyril
 * @since 1.3.0 2010-01-17
 */
public class TxCountPlotBuilder extends AbstractPlotBuilder implements PlotBuilder {

	private TimeSeries series = new TimeSeries("TX Count");
	...
	public XYPlot buildPlot() {
		...
		for (int i = 0; i < mavDays.length; i++) {
			...
			TimeSeries mav = MovingAverage.createMovingAverage(series,
					"TX Count (" + mavDay + "-Day MA)", mavDay, mavDay);
