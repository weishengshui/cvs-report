...
/**
 * This report is created for the activity of CocoPark商圈活动报表
 *
 */
...
String sTransdateFrom = "2010-06-18";	// should be 2010-03-16
String sTransdateTo = "2010-07-31";		// should be 2010-06-01, exclusive
// sHighlightTransaction: For transactions made on or after this day, 
// they will be highlighted with color
String sHighlightTransactionSince = "2010-06-01";
...
double highlightXactionAmount1 = 100.0;
double highlightXactionAmount2 = 500.0;
double highlightXactionAmount3 = 2000.0;
...
try {
	...
	if (shopFilter == null || shopFilter.length() == 0) {
		out.println("找不到" + targetBizCircleName + "商圈.");
		return;
	}
...
<li>活动对象：活动时间内消费的会员。</li>
<li>需要的是在活动时间内的会员在活动期间的消费记录。</li>
</ul>
...
<tr>
<td>2010-06-25 09:35</td>
<td>CocoPark商圈活动报表的改动.增加积分券消费记录显示。(jessica需求)</td>
</tr>
<tr>
<td>2010-06-22</td>
<td>CocoPark商圈活动报表。</td>
</tr>
</table>
<br/>
<hr/>
...
<td class="xact_lv3">&gt;=<%=highlightXactionAmount3%>元</td>
</tr>
</table>
<br><br>
<table>
...
sqlMatchActivityTarget += "SELECT DISTINCT tempmembertxid FROM clubpoint WHERE " + spendingFilter + " AND  shopid IN (" + shopFilter + ") or  merchantname in ('深圳市福田区银田料理店')";
...
while (rs.next()) {
	...
	if (!rs.getTimestamp("transdate").before(xactDate)) {
		...
		if (amt >= highlightXactionAmount3) {
			rowCssClass = "xact_lv3";
		} else if (amt >= highlightXactionAmount2) {
...
<td<%=tdCssClass%>><%=JspDisplayUtil.noNull(rs.getString("TEMPMEMBERTXID"))%></td>
