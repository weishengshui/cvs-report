...
<%
	...
	PreparedStatement pstmtTransactionRefno = null;
	ResultSet rsMember = null;
	ResultSet rsRefno = null;
	...
	String targetMerchantBizName = "丁畅制造";

	String targetMerchantId = null;
	List<String> targetShopIds = new ArrayList<String>();

	String sTransdateFrom = "2010-09-05"; // should be 2010-03-16
	String sTransdateTo = "2010-10-01"; // should be 2010-06-01, exclusive
	// sHighlightTransaction: For transactions made on or after this day, 
	// they will be highlighted with color
	String sHighlightTransactionSince = "2010-09-05";
...
<ul>
	...
	<li>活动对象：活动时间内在丁畅制造消费的会员。</li>
	<li>需要的是在活动时间内在丁畅制造消费的会员在活动期间的消费记录。</li>
</ul>
...
<table>
		...
		<%
			conn = DbConnectionFactory.getInstance()
				...
				String spendingFilter = "((clubid = '00' AND amountcurrency > 2 AND transdate >= to_date('"
						...
						+ sTransdateTo + "','yyyy-mm-dd')) or (clubid = '00'  AND point = 0 and transdate >= to_date('2010-10-09 00:00:01','yyyy-mm-dd hh24:mi:ss') and transdate <= to_date('2010-10-09 23:59:59','yyyy-mm-dd hh24:mi:ss')))";
				...
				pstmtTransactionRefno = conn.prepareStatement("select txrefno from transaction where clubpoint_id = ?");
				...
				sqlMatchActivityTarget += "SELECT DISTINCT tempmembertxid FROM clubpoint WHERE "
						...
						+ shopFilter
						+ ") ";
				sqlMatchActivityTarget += ") and membercardid like '95519%'";
				...
				while (rs.next()) {
					...
					pstmtTransactionRefno.setString(1,id);
					rsRefno = pstmtTransactionRefno.executeQuery();
					String refno = null;
					
					if(rsRefno.next())
					{
						refno = rsRefno.getString("txrefno");
					}
					
					SqlUtil.close(rsRefno);
 ...
 %>
			...
			<td <%=tdCssClass%> style="text-align: right"><%=refno %></td>
		</tr>
		<%
...
<%
	SqlUtil.close(rs);
		...
		SqlUtil.close(pstmtTransactionRefno);
		// connections
		SqlUtil.close(conn);
