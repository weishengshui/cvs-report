...
<%
	...
	GregorianCalendar gc = new GregorianCalendar(2010, 10, 15);
	java.sql.Date startDate = new java.sql.Date(gc.getTimeInMillis());

	GregorianCalendar gc1 = new GregorianCalendar(2010, 11, 11);
...
<%
	...
	try {
		...
		List<ConsumeData> dataList = service.getAllConsumeData(shopIds,
				startDate, endDate, posIds);

		if ("9581".equals(shopIds[0])) {
			out.println("<h2>大饱口福消费记录实时报表:</h2>");

		} else {
			out.println("<h2>四海一家消费记录实时报表:</h2>");
		}
